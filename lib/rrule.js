...
;(function (root, factory) {
  ...
  var dateutil = {
    ...
    /*
    * Parse Duration and output total seconds
    */
    durationToSeconds: function(duration){
        var seconds = 0;
        var isPositive = true;
        var indexOfP = duration.indexOf("P");
        if(indexOfP < 0) throw new Error("Invalid DURATION value: "+duration);
        if(indexOfP == 1){
            //get sign
            if(duration[0]=="-") isPositive = false;
        }
        var indexOfWeeks = duration.indexOf("W");
        if(indexOfWeeks > -1){
            seconds = seconds + duration[indexOfWeeks-1] * 60 * 60 * 24 * 7;
        }
        var indexOfDays = duration.indexOf("D");
        if(indexOfDays > -1){
            seconds = seconds + duration[indexOfDays-1] * 60 * 60 * 24;
        }
        var indexOfHours = duration.indexOf("H");
        if(indexOfHours > -1){
            seconds = seconds + duration[indexOfHours-1] * 60 * 60;
        }
        var indexOfMinutes = duration.indexOf("M");
        if(indexOfMinutes > -1){
            seconds = seconds + duration[indexOfMinutes-1] * 60 ;
        }
        var indexOfSeconds = duration.indexOf("S");
        if(indexOfSeconds > -1){
            seconds = seconds + duration[indexOfSeconds-1] ;
        }
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      switch (key) {
        ...
        case 'UNTIL':
          ...
          break
        case 'DURATION':
          value = dateutil.durationToSeconds(value)
          break
        default:
          if (value instanceof Array) {
    ...
    for (i = 0; i < attrs.length; i++) {
      ...
      switch (key) {
        ...
        case 'BYEASTER':
          ...
          break
        case 'DURATION':
          options.duration = dateutil.durationToSeconds(value)
          break
        default:
